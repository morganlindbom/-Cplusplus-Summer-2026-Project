...
QString findOpenOcd()
{
    ...
    const QStringList paths = {env.value("PICO_OPENOCD_PATH"),
                               QDir::home().filePath(".pico-sdk/openocd/0.12.0+dev/openocd.exe"),
                               QStringLiteral("C:/Program Files/Raspberry Pi/Pico SDK v1.5.1/openocd/openocd.exe")};
...
TransferColumn3::TransferColumn3(const QString& db, QWidget* parent) : QWidget(parent)
{
    ...
    drive_ = new QLineEdit("E:/", this);
    f->addRow("UF2 drive (copy method)", drive_);
    l->addLayout(f);
    auto* go = new QPushButton("Transfer Firmware", this);
    go->setObjectName("transfer_firmware");
    l->addWidget(go);
    ...
    connect(process_, &QProcess::readyReadStandardOutput, this,
            [this]() { log_->appendPlainText(QString::fromLocal8Bit(process_->readAllStandardOutput())); });
    connect(process_, &QProcess::readyReadStandardError, this,
            [this]() { log_->appendPlainText(QString::fromLocal8Bit(process_->readAllStandardError())); });
    connect(go, &QPushButton::clicked, this,
            [this]()
            {
                ...
                if (method_ == "OpenOCD probe")
                {
                    ...
                    process_->start(ocd, {"-s", scripts, "-f", "interface/cmsis-dap.cfg", "-f", "target/rp2350.cfg",
                                          "-c", "adapter speed 5000", "-c", programCommand});
                    log_->appendPlainText("Flashing through CMSIS-DAP probe: " + elf);
                }
                else if (method_.startsWith("picotool"))
                    process_->start("picotool", {"load", "-f", artifact_});
                else
                {
                    const QString target = QDir(drive_->text()).filePath(QFileInfo(artifact_).fileName());
                    if (QFile::copy(artifact_, target))
                        log_->appendPlainText("Copied to " + target);
                    else
                        log_->appendPlainText("Copy failed. Check drive path and BOOTSEL mode.");
                }
            });
}
