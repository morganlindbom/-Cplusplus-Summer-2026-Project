...
QList<int> columnSizesForWidth(int width)
{
    return {int(width * kWorkflowColumnPercent), int(width * kProjectSettingsColumnPercent),
            int(width * kProjectManagementColumnPercent), int(width * kViewerColumnPercent)};
}
} // namespace

namespace pvd
{
MainWindow::MainWindow(Workflow* workflow, Viewer* viewer, QWidget* parent)
    : QMainWindow(parent), workflow_(workflow), viewer_(viewer)
{
    ...
    column2Stack_->setObjectName("workflow_column2_stack");
    column3Stack_ = new QStackedWidget(splitter);
    column3Stack_->setObjectName("workflow_column3_stack");
    ...
    QTimer::singleShot(0, this, [splitter]() { splitter->setSizes(columnSizesForWidth(splitter->width())); });
    setStyleSheet(
        "QMainWindow{background:#f4f6f8;} QWidget{font-family:'Segoe UI';font-size:12px;} "
        "QListWidget,QPlainTextEdit,QTextEdit,QTableWidget,QTreeWidget{background:white;border:1px solid #d7dce2;} "
        "QPushButton{padding:5px 8px;} QGroupBox{font-weight:600;} QSplitter::handle{background:#e4e7eb;width:3px;}");
}
