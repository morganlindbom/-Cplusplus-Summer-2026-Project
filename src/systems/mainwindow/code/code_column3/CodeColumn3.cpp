...
CodeColumn3::CodeColumn3(const QString& db,QWidget* parent):QWidget(parent)
{
    ...
    auto* l=makePanelLayout(this,SqliteUtil::metadata(db,"title","Generated C++ / C Source"));pathLabel_=new QLabel(this);pathLabel_->setWordWrap(true);l->addWidget(pathLabel_);editor_=new QPlainTextEdit(this);editor_->setLineWrapMode(QPlainTextEdit::NoWrap);l->addWidget(editor_,1);auto* buttons=new QHBoxLayout();auto* save=new QPushButton("Save File",this);auto* check=new QPushButton("Check Syntax",this);auto* build=new QPushButton("Build / Show Errors",this);buttons->addWidget(save);buttons->addWidget(check);buttons->addWidget(build);l->addLayout(buttons);status_=new QLabel(this);status_->setWordWrap(true);l->addWidget(status_);connect(save,&QPushButton::clicked,this,[this](){/**Commits editor text to the selected generated file.*/saveCurrent();status_->setText("File saved.");});connect(check,&QPushButton::clicked,this,&CodeColumn3::checkSyntax);connect(build,&QPushButton::clicked,this,[this](){saveCurrent();emit buildRequested();status_->setText("Build started. Errors will be shown here when the build finishes.");});
}
...
void CodeColumn3::checkSyntax()
{
        ...
        for(const QChar ch:raw)
        {
            ...
            else if(ch=='}'||ch==')'||ch==']')
            {
                if(stack.isEmpty())
                {
                    ...
                    status_->setText(QString("Syntax error near line %1: closing bracket without an opening bracket.").arg(line+1));
                    return;
                }
                const QChar open=stack.last().first;
                const bool matches=(open=='{'&&ch=='}')||(open=='('&&ch==')')||(open=='['&&ch==']');
                if(!matches)
                {
                    status_->setStyleSheet("color:#a32626;");
    ...
    {
        status_->setStyleSheet("color:#a32626;");
        status_->setText(QString("Syntax error: opening '%1' from line %2 is not closed.").arg(stack.last().first).arg(stack.last().second));
        return;
    }
...
{
    status_->setStyleSheet(ok?"color:#126b2f;":"color:#a32626;");status_->setText(ok?"Build succeeded.":"Build failed:\n"+message);
}
}
