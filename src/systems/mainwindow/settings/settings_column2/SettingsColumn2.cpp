...
SettingsColumn2::SettingsColumn2(const QString& db,QWidget* parent):QWidget(parent)
{
    ...
    auto* l=makePanelLayout(this,SqliteUtil::metadata(db,"title","Selected Functions"));list_=new QListWidget(this);l->addWidget(list_,1);connect(list_,&QListWidget::currentItemChanged,this,[this](QListWidgetItem* item){/**Requests Settings Column 3 context from System.*/if(item)emit selectionRequested(item->data(Qt::UserRole).toString());});
}
