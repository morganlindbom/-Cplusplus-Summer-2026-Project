...
DebugColumn3::DebugColumn3(const QString& db, QWidget* parent) : QWidget(parent)
{
    ...
    auto* start = new QPushButton("Start OpenOCD", this);
    ...
    connect(process_, &QProcess::errorOccurred, this, [this](QProcess::ProcessError error)
            { log_->appendPlainText("OpenOCD process error: " + QString::number(static_cast<int>(error))); });
...
void DebugColumn3::configure(const QString& exe, const QString& interfaceCfg, const QString& targetCfg, int speed,
                             const QString& resetMethod)
{
    exe_ = exe;
    interface_ = interfaceCfg;
    target_ = targetCfg;
    speed_ = speed;
