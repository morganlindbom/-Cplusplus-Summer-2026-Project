...
namespace
{
QString settingDetails(const QString& key, const QString& label, const QString& defaultValue, const QString& help,
                       const QStringList& values)
{
    /**Builds the explanatory text shown for one visible setting.*/
    QString example =
        QString("Exempel: standardvärdet är %1.").arg(defaultValue.isEmpty() ? QStringLiteral("tomt") : defaultValue);
    QString alternative =
        values.isEmpty()
            ? QStringLiteral(
                  "Alternativ användning: välj ett annat värde om din hårdvara eller applikation kräver det.")
            : QString("Alternativ användning: %1.").arg(values.join(", "));
    QString warning =
        QStringLiteral("Varning: ändra värdet försiktigt och kontrollera att det stöds av RP2350A och vald funktion.");
    QString recommended =
        values.isEmpty()
            ? QStringLiteral("Använd standardvärdet som startpunkt och håll dig inom hårdvarans dokumenterade gränser.")
            : QString("Rekommenderade val: %1.").arg(values.join(", "));
    if (key == "frequency_hz")
    {
    ...
    if (key == "frequency_hz")
    {
        ...
        alternative = QStringLiteral("Alternativ användning: lägre frekvens passar LED och motorstyrning; högre "
                                     "frekvens passar ljud eller snabb reglering.");
    ...
    else if (key == "duty_percent")
    {
        ...
        alternative = QStringLiteral("Alternativ användning: använd 50 % som neutral testnivå eller ändra värdet "
                                     "dynamiskt i den färdiga koden.");
        warning = QStringLiteral(
            "Varning: 0 % är alltid av och 100 % är alltid på; polaritetsinvertering kan vända resultatet.");
    }
    else if (key == "wrap")
    {
        example = QStringLiteral("Exempel: wrap 1000 ger 1001 räknarsteg och påverkar PWM-upplösningen.");
        alternative = QStringLiteral(
            "Alternativ användning: höj wrap för finare duty-upplösning eller sänk den för högre möjlig frekvens.");
        warning = QStringLiteral("Varning: wrap måste kombineras med frekvens och dividerare; extrema värden kan ge "
                                 "avrundning eller för låg upplösning.");
    }
    else if (key == "clock_divider")
    {
        example = QStringLiteral("Exempel: divider 5.0 delar 125 MHz-klockan till ungefär 25 MHz.");
        alternative = QStringLiteral("Alternativ användning: använd Manual timing för exakt dividerare, eller "
                                     "Automatic timing för att låta frekvensen styra den.");
        warning = QStringLiteral(
            "Varning: giltigt område är ungefär 1.0–256.0; för stor dividerare begränsar frekvens och upplösning.");
    }
    else if (key == "phase_correct")
    {
        ...
        warning = QStringLiteral(
            "Varning: fas-korrekt läge halverar ungefär maximal frekvens vid samma wrap och dividerare.");
    }
    else if (key == "invert_a" || key == "invert_b")
    {
        example = QStringLiteral("Exempel: aktivera för att göra en aktiv låg utgång av kanalen.");
        alternative = QStringLiteral(
            "Alternativ användning: använd invertering när drivsteget eller extern gate har omvänd logik.");
        warning = QStringLiteral("Varning: invertering påverkar polariteten, inte duty-värdet; kontrollera alltid vad "
                                 "som är säkert för ansluten last.");
    }
    else if (key == "system_clock_khz")
    {
        example = QStringLiteral("Exempel: 150000 kHz ställer systemklockan till 150 MHz.");
        alternative = QStringLiteral(
            "Alternativ användning: använd 0 för SDK-standard eller välj lägre klocka för lägre effekt och värme.");
        warning = QStringLiteral(
            "Varning: överklockning kan kräva högre spänning, ge instabilitet och öka effektförbrukningen.");
    }
    else if (key == "vreg_voltage")
    {
        example = QStringLiteral("Exempel: 1.10 V är en balanserad standardnivå för normal systemklocka.");
        alternative = QStringLiteral("Alternativ användning: lägre spänning kan minska effekt; högre spänning kan "
                                     "behövas vid högre klockfrekvens.");
        warning = QStringLiteral("Varning: fel spänning kan orsaka instabilitet eller skada. Höj aldrig spänningen "
                                 "utan att verifiera RP2350A-gränserna.");
    }
    else if (key == "core1_enabled")
    {
        example = QStringLiteral("Exempel: aktiverad startar core 1 en separat bakgrundsloop.");
        alternative = QStringLiteral(
            "Alternativ användning: lämna av för ett enklare single-core-program och mindre resursanvändning.");
        warning = QStringLiteral(
            "Varning: core 0 och core 1 delar minne och resurser; synkronisering krävs i egen applikationskod.");
    }
    else if (key == "watchdog_enabled" || key == "watchdog_timeout_ms")
    {
        ...
        alternative = QStringLiteral(
            "Alternativ användning: stäng av under felsökning eller använd kortare timeout i säkerhetskritisk drift.");
        warning = QStringLiteral(
            "Varning: en aktiv watchdog kan starta om systemet under långvariga operationer eller debugging.");
    }
    else if (key == "stdio_usb" || key == "stdio_uart")
    {
        ...
        alternative =
            QStringLiteral("Alternativ användning: UART stdio passar för seriell konsol när USB inte är tillgänglig.");
        warning = QStringLiteral("Varning: stdio-kanaler använder resurser och kan påverka starttid eller vilka "
                                 "pinnar/periferier som är lediga.");
    }
    else if (key == "uart_baud")
    {
        example = QStringLiteral("Exempel: 115200 baud är ett vanligt värde för seriell kommunikation.");
        alternative = QStringLiteral("Alternativ användning: välj 9600 för långsammare utrustning eller högre "
                                     "hastighet om mottagaren stöder det.");
        warning =
            QStringLiteral("Varning: sändare och mottagare måste använda samma baud rate, annars blir data oläsbar.");
    }
    else if (key == "startup_delay_ms")
    {
        example = QStringLiteral("Exempel: 1000 ms väntar en sekund efter initiering innan applikationen fortsätter.");
        alternative = QStringLiteral(
            "Alternativ användning: använd för att ge USB, sensorer eller externa kretsar tid att starta.");
        warning = QStringLiteral("Varning: en lång fördröjning kan se ut som att kortet hängt sig vid uppstart.");
    }
    return QString("<b>%1</b><br><br>%2<br><br><b>Rekommenderat "
                   "min/max</b><br>%3<br><br><b>Exempel</b><br>%4<br><br><b>Alternativ "
                   "användning</b><br>%5<br><br><b>Varning</b><br>%6")
        .arg(label, help, recommended, example, alternative, warning);
}
} // namespace
SettingsColumn3::SettingsColumn3(const QString& db, FunctionCatalog* catalog, QWidget* parent)
    : QWidget(parent), catalog_(catalog)
{
    /**Builds the settings form and its function-information panel.*/
...
void SettingsColumn3::addSetting(const QString& key, const QString& label, const QString& type,
                                 const QString& defaultValue, const QString& help, const QStringList& values)
{
    /**Adds one typed setting editor and routes changes back to the active selection.*/
    ...
    labelButton->setStyleSheet("QPushButton { text-align: left; border: none; padding: 2px; color: palette(text); } "
                               "QPushButton:hover { text-decoration: underline; }");
    connect(labelButton, &QPushButton::clicked, this, [this, details]() { info_->setText(details); });
    if (type == "bool")
    {
        ...
        connect(c, &QCheckBox::toggled, this,
                [this, key, details](bool v)
                {
                    emit settingChanged(componentId_, key, v ? "true" : "false");
    ...
    else if (type == "int")
    {
        ...
        connect(c, qOverload<int>(&QComboBox::activated), this,
                [c](int index)
                {
                    if (index == c->count() - 1 && c->itemText(index) == "Custom...")
                    {
                        c->setEditText(QString{});
                        c->lineEdit()->setFocus();
                    }
                });
        editor = c;
...
void SettingsColumn3::showSelection(const FunctionSelection& selection)
{
    ...
    {
        addSetting("execution_core", "Execution core", "combo", "Core 0",
                   FunctionExecutionModel::coreSelectionReason(functionId_), {"Core 0", "Core 1"});
    }
    else if (core1Enabled_ && functionId_ != "rp2350a.configure" && functionId_ != "debug_probe.cmsis_dap")
    {
        auto* coreReason =
            new QLabel("Core 0 only — " + FunctionExecutionModel::coreSelectionReason(functionId_), this);
    ...
    if (!db.isEmpty())
    {
                                      ...
                                      "settings ORDER BY sort_order"))
        {
            ...
            addSetting(row.value("setting_key").toString(), row.value("label").toString(),
                       ...
                       row.value("enum_values").toString().split('|', Qt::SkipEmptyParts));
        }
        if (selection.functionId.startsWith("pwm"))
        {
            addSetting("timing_mode", "Timing mode", "combo", "Automatic",
                       "Automatic uses frequency and wrap; Manual uses the divider directly.", {"Automatic", "Manual"});
            addSetting("wrap", "Counter top / wrap", "int", "1000",
                       "PWM counter TOP value. Resolution is wrap + 1 steps.");
            addSetting("clock_divider", "Clock divider", "text", "1.0",
                       "PWM clock divider from 1.0 to 256.0. Used in Manual timing mode.");
            addSetting("divider_mode", "Divider mode", "combo", "Free-running", "How the PWM clock divider advances.",
                       {"Free-running", "B rises", "B high"});
        ...
        if (selection.functionId.startsWith("pio"))
        {
            ...
            {
                addSetting("pin_count", "Pin count", "int", "8",
                           "Number of consecutive pins controlled by the PIO state machine.");
                addSetting("base_pin", "Base GPIO", "int", QString::number(selection.gpio),
                           "First GPIO in the PIO pin range.");
                addSetting("shift_direction", "Shift direction", "combo", "Right", "Bit shift direction.",
                           {"Left", "Right"});
                addSetting("autopush", "Auto-push", "bool", "false",
                           "Automatically push RX data at the selected threshold.");
                addSetting("autopull", "Auto-pull", "bool", "false",
                           "Automatically pull TX data at the selected threshold.");
            ...
            addSetting("dead_time_cycles", "Dead time (PIO cycles)", "int", "0",
                       "Extra PIO delay inserted in the program loop. 0 disables dead time.");
        }
    }
    else if (selection.functionId == "rp2350a.configure")
    {
        ...
        addSetting("system_clock_khz", "System clock (kHz)", "int", "150000",
                   "RP2350A system clock frequency. Set 0 to keep the SDK default.");
        addSetting("vreg_voltage", "Core voltage (V)", "combo", "1.10",
                   "Core voltage used before changing the system clock.",
                   {"0.85", "0.90", "0.95", "1.00", "1.05", "1.10", "1.15", "1.20", "1.25", "1.30"});
        addSetting("core1_enabled", "Enable core 1", "bool", "false",
                   "Launch a background loop on the second RP2350A core.");
        addSetting("watchdog_enabled", "Enable watchdog", "bool", "false", "Enable the RP2350A watchdog timer.");
        addSetting("watchdog_timeout_ms", "Watchdog timeout (ms)", "int", "2000",
                   "Watchdog timeout before an automatic reset.");
    ...
    else if (selection.functionId == "onboard_led.output")
    {
        ...
        addSetting("blink_enabled", "Blink LED", "bool", "false",
                   "Run non-blocking LED blinking on the selected execution core.");
    ...
    else if (selection.functionId == "wireless.cyw43")
    {
        ...
        addSetting("wifi_mode", "Wi-Fi mode", "combo", "Station", "Wireless operating mode.",
    ...
    else if (selection.functionId == "debug_probe.cmsis_dap")
    {
        ...
        addSetting("reset_method", "Reset method", "combo", "run reset", "Debug-session reset method.",
                   {"run reset", "halt reset", "none"});
    }
    else if (selection.functionId == "robo.neopixel")
    {
        info_->setText("ROBO-PICO NeoPixel template\n\nWS2812 RGB LED on GPIO18 from robo_pico_source_of_truth.jsonc. "
                       "Pixel count 2 supports two serially connected LEDs.");
    ...
    else if (selection.functionId == "robo.buzzer")
    {
        ...
        addSetting("repeat_interval_ms", "Tone repeat interval (ms)", "int", "2000",
                   "Time from one tone start to the next tone start.");
    }
    else
    {
        info_->setText(selection.functionName);
