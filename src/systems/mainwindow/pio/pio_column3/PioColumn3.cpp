...
PioColumn3::PioColumn3(const QString& db, QWidget* parent) : QWidget(parent)
{
    ...
    auto* l = makePanelLayout(this, SqliteUtil::metadata(db, "title", "PIO Source"));
    editor_ = new QPlainTextEdit(this);
    editor_->setLineWrapMode(QPlainTextEdit::NoWrap);
    l->addWidget(editor_, 1);

    auto* row = new QHBoxLayout();
    auto* generate = new QPushButton("Generate PIO Code", this);
    auto* validateButton = new QPushButton("Validate PIO Code", this);
    auto* restore = new QPushButton("Restore Generated PIO Code", this);
    row->addWidget(generate);
    row->addWidget(validateButton);
    ...
    connect(editor_, &QPlainTextEdit::textChanged, this,
            [this]()
            {
                /**Stores current PIO source in memory.*/
                if (!current_.isEmpty())
                {
                    programs_[current_] = editor_->toPlainText();
    ...
    connect(generate, &QPushButton::clicked, this,
            [this]()
            {
                /**Replaces the current buffer with a generated skeleton.*/
                if (!current_.isEmpty())
                    editor_->setPlainText(generatedTemplate(current_));
            });
    connect(restore, &QPushButton::clicked, this,
            [this]()
            {
                /**Restores the deterministic generated PIO skeleton.*/
                if (!current_.isEmpty())
                    editor_->setPlainText(generatedTemplate(current_));
            });
    connect(validateButton, &QPushButton::clicked, this,
            [this]()
            {
                /**Runs lightweight structural PIO validation.*/
                QString e;
                status_->setText(validate(&e) ? "PIO validation: PASS" : "PIO validation: " + e);
            });
}
void PioColumn3::selectProgram(const QString& name)
{
    ...
    if (!programs_.contains(name))
        programs_[name] = generatedTemplate(name);
    editor_->setPlainText(programs_[name]);
}
void PioColumn3::reloadGeneratedFiles(const QStringList& files)
{
    ...
    {
        if (QFileInfo(path).suffix().toLower() != "pio")
            continue;
        QFile file(path);
        if (file.open(QIODevice::ReadOnly | QIODevice::Text))
            generated.insert(QFileInfo(path).completeBaseName(), QString::fromUtf8(file.readAll()));
    }
    if (generated.isEmpty())
        return;
    programs_ = generated;
    if (!programs_.contains(current_))
        current_ = programs_.constBegin().key();
    if (!current_.isEmpty())
    {
        const QSignalBlocker blocker(editor_);
        editor_->setPlainText(programs_.value(current_));
...
bool PioColumn3::validate(QString* error) const
{
    ...
    if (!t.contains(".wrap_target"))
    {
        if (error)
            *error = "missing .wrap_target";
        return false;
    }
    if (!t.contains(".wrap"))
    {
