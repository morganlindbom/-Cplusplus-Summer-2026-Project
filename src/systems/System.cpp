...
namespace pvd
{
    ...
    void System::connectComponents()
    {
        ...
        connect(settings3_, &SettingsColumn3::settingChanged, this, [this](const QString &id, const QString &key, const QString &value)
                {if(state_.selections.contains(id)){state_.selections[id].settings[key]=value;ProjectStore::save(state_);if(id=="debug_probe")debug2_->applySetting(key,value);} });
        ...
        connect(project3_, &ProjectColumn3::createRequested, this, [this](const QString &name, const QString &path)
                {state_.projectName=name.trimmed().isEmpty()?"PICO2W":name;state_.projectPath=path;QString error;if(!ProjectStore::save(state_,&error))QMessageBox::warning(window_,"Create Project",error);refreshProjectViews(); });
        connect(project3_, &ProjectColumn3::openRequested, this, [this](const QString &path)
                {QString error;if(!ProjectStore::load(path,&state_,&error))QMessageBox::warning(window_,"Open Project",error);refreshProjectViews(); });
        connect(project3_, &ProjectColumn3::saveRequested, this, [this]()
                {QString error;if(!ProjectStore::save(state_,&error))QMessageBox::warning(window_,"Save Project",error);refreshProjectViews(); });
        ...
        connect(workflow_, &Workflow::workflowSelected, this, [this](const QString &id)
                {if(id=="build"){const QString source=QDir(state_.projectPath).filePath("generated"),build=QDir(state_.projectPath).filePath("build");build2_->setProjectPath(state_.projectPath);build3_->setPaths(source,build);}else if(id=="transfer"){const QString build=QDir(state_.projectPath).filePath("build");transfer2_->setBuildPath(build);transfer3_->configure(transfer2_->artifact(),transfer2_->method());}else if(id=="debug"){applyDebugProbeSettings();debug3_->configure(debug2_->openocd(),debug2_->interfaceCfg(),debug2_->targetCfg(),debug2_->speed(),debug2_->resetMethod());} });
    }
