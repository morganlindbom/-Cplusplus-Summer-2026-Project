...
namespace pvd
{
    ...
    void System::connectComponents()
    {
        ...
        connect(viewer_, &Viewer::componentSelected, this, [this](const QString &id)
                { window_->activateWorkflow("function_selection"); });
        connect(workflow_, &Workflow::workflowSelected, window_, &MainWindow::activateWorkflow);
        connect(function2_, &FunctionSelectionColumn2::componentSelected, this, [this](const QString &id)
                {function3_->setComponent(id);viewer_->selectComponent(id);if(state_.selections.contains(id))function3_->setSelectedFunction(state_.selections[id].functionId); });
        connect(viewer_, &Viewer::componentSelected, this, [this](const QString &id)
                { function2_->selectComponent(id); });
        connect(function3_, &FunctionSelectionColumn3::functionChanged, this, &System::handleFunctionChange);
        connect(settings2_, &SettingsColumn2::selectionRequested, this, [this](const QString &id)
                {if(state_.selections.contains(id))settings3_->showSelection(state_.selections[id]); });
        connect(settings3_, &SettingsColumn3::settingChanged, this, [this](const QString &id, const QString &key, const QString &value)
                {if(state_.selections.contains(id)){state_.selections[id].settings[key]=value;latestBuildSuccessful_=false;if(id=="rp2350a"&&(key=="core1_enabled"||key=="enabled")){const auto& rp=state_.selections[id];settings3_->setCore1Enabled(rp.settings.value("enabled","true")=="true"&&rp.settings.value("core1_enabled","false")=="true");settings3_->showSelection(rp);}ProjectStore::save(state_);if(id=="debug_probe")debug2_->applySetting(key,value);} });
        connect(pio3_, &PioColumn3::programsChanged, this, [this]()
                {latestBuildSuccessful_=false;pio2_->setPrograms(pio3_->programs().keys()); });
        connect(project2_, &ProjectColumn2::languageChanged, this, [this](const QString &value)
                { state_.language = value;latestBuildSuccessful_=false; });
        connect(project2_, &ProjectColumn2::stateChanged, this, [this](const QString &value)
                { state_.state = value; });
        connect(project2_, &ProjectColumn2::productChanged, this, [this](const QString &value)
                { state_.product = value; });
        connect(project2_, &ProjectColumn2::debugSessionToolsChanged, this, [this](bool value)
                { state_.debugSessionTools = value; });
        connect(project2_, &ProjectColumn2::runtimeDiagnosticsChanged, this, [this](bool value)
                { state_.runtimeDiagnostics = value; });
        connect(project2_, &ProjectColumn2::verboseBuildEvidenceChanged, this, [this](bool value)
                { state_.verboseBuildEvidence = value; });
        connect(project3_, &ProjectColumn3::createRequested, this, [this](const QString &name, const QString &path)
                {state_.projectName=name.trimmed().isEmpty()?"PICO2W":name;state_.projectPath=path;latestBuildSuccessful_=false;lastBuiltProjectPath_.clear();QString error;const bool ok=ProjectStore::save(state_,&error);if(!ok)QMessageBox::warning(window_,"Create Project",error);project3_->setStatus(ok?"Project saved in: "+path:error,ok);refreshProjectViews(); });
        connect(project3_, &ProjectColumn3::openRequested, this, [this](const QString &path)
                {latestBuildSuccessful_=false;lastBuiltProjectPath_.clear();QString error;if(!ProjectStore::load(path,&state_,&error))QMessageBox::warning(window_,"Open Project",error);refreshProjectViews(); });
        connect(project3_, &ProjectColumn3::saveRequested, this, [this](const QString &name, const QString &path)
                {if(state_.projectName!=name||QDir(state_.projectPath).absolutePath()!=QDir(path).absolutePath()){latestBuildSuccessful_=false;lastBuiltProjectPath_.clear();}state_.projectName=name;state_.projectPath=path;QString error;const bool ok=ProjectStore::save(state_,&error);if(!ok)QMessageBox::warning(window_,"Save Project",error);project3_->setStatus(ok?"Project saved in: "+path:error,ok);refreshProjectViews(); });
        connect(generate3_, &GenerateColumn3::validateRequested, this, [this]()
                {QString report;const bool ok=ProjectGenerator::validate(state_,&report);generate3_->setStatus(report,ok); });
        connect(generate3_, &GenerateColumn3::generateRequested, this, [this]()
                {latestBuildSuccessful_=false;QString error;if(ProjectGenerator::generate(&state_,pio3_->programs(),&error)){pio3_->reloadGeneratedFiles(state_.generatedFiles);ProjectStore::save(state_);generate3_->setStatus(QString("Ready for Build — Generation completed: %1 files").arg(state_.generatedFiles.size()),true);refreshProjectViews();}else generate3_->setStatus(error,false); });
        connect(code2_, &CodeColumn2::fileSelected, code3_, &CodeColumn3::loadFile);
        connect(code3_, &CodeColumn3::buildRequested, this, [this]()
                {const QString source=QDir(state_.projectPath).filePath("generated"),build=QDir(state_.projectPath).filePath("build");build2_->setProjectPath(state_.projectPath);build3_->setPaths(source,build);build3_->build();});
        connect(build3_, &BuildColumn3::buildStarted, this, [this](){latestBuildSuccessful_=false;});
        connect(build3_, &BuildColumn3::buildCompleted, this, [this](bool ok, const QString& output)
                {latestBuildSuccessful_=ok;lastBuiltProjectPath_=ok?QDir(state_.projectPath).canonicalPath():QString{};code3_->setBuildResult(ok, output);});
        connect(pio2_, &PioColumn2::programSelected, pio3_, &PioColumn3::selectProgram);
        connect(workflow_, &Workflow::workflowSelected, this, [this](const QString &id)
                {if(id=="build"){const QString source=QDir(state_.projectPath).filePath("generated"),build=QDir(state_.projectPath).filePath("build");build2_->setProjectPath(state_.projectPath);build3_->setPaths(source,build);}else if(id=="transfer"){const QString generated=QDir(state_.projectPath).filePath("generated"),build=QDir(state_.projectPath).filePath("build");QString target=state_.projectName;target.replace(QRegularExpression("[^A-Za-z0-9_]"),"_");if(target.isEmpty())target="PICO2W";transfer2_->setBuildPath(build,target);const bool currentBuild=latestBuildSuccessful_&&lastBuiltProjectPath_==QDir(state_.projectPath).canonicalPath();transfer3_->configure(transfer2_->artifact(),transfer2_->method(),generated,build,target,currentBuild);}else if(id=="debug"){applyDebugProbeSettings();debug3_->configure(debug2_->openocd(),debug2_->interfaceCfg(),debug2_->targetCfg(),debug2_->speed(),debug2_->resetMethod());} });
    }
    void System::handleFunctionChange(const QString &componentId, const FunctionOption &option, int gpio, int physicalPin)
    {
        ...
        viewer_->setSimulationText(option.id == "disabled" ? "Disabled: " + componentDisplayName(componentId) : componentDisplayName(componentId) + " — " + option.name);
    }
    void System::refreshProjectViews()
    {
        ...
        settings3_->setCore1Enabled(rp.functionId=="rp2350a.configure"&&rp.settings.value("enabled","true")=="true"&&rp.settings.value("core1_enabled","false")=="true");
        project2_->setProjectState(state_.product, state_.language, state_.state,
                                   state_.debugSessionTools, state_.runtimeDiagnostics,
