...
FunctionSelectionColumn2::FunctionSelectionColumn2(const QString& db,QWidget* parent):QWidget(parent)
{
    ...
    auto* l=makePanelLayout(this,SqliteUtil::metadata(db,"title","Components"));list_=new QListWidget(this);list_->setSelectionMode(QAbstractItemView::SingleSelection);const QStringList special={"rp2350a","wireless","usb_connector","bootsel","onboard_led","debug_probe"};for(const auto& id:special){auto* item=new QListWidgetItem(componentDisplayName(id),list_);item->setData(Qt::UserRole,id);item->setData(Qt::AccessibleTextRole,componentDisplayName(id));}for(int p=1;p<=40;++p){auto* item=new QListWidgetItem(QString("Pin %1").arg(p),list_);item->setData(Qt::UserRole,QString("pin_%1").arg(p));item->setData(Qt::AccessibleTextRole,QString("Pin %1").arg(p));}l->addWidget(list_,1);
    const auto emitComponent=[this](QListWidgetItem* item){/**Routes every normal row activation through one stable component-selection path.*/if(!item)return;const QString id=item->data(Qt::UserRole).toString();if(id.isEmpty())return;emit componentSelected(id);};
    connect(list_,&QListWidget::currentItemChanged,this,[emitComponent](QListWidgetItem* item,QListWidgetItem*){emitComponent(item);});
