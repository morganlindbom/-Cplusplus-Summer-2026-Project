...
BuildColumn3::BuildColumn3(const QString& db, QWidget* parent) : QWidget(parent)
{
    ...
    connect(configure, &QPushButton::clicked, this,
            [this]()
            {
                ...
                run("cmake",
                    {"-S", source_, "-B", build_, "-G", "Ninja", "-DPICO_BOARD=pico2_w",
...
{
    run("cmake", {"--build", build_, "--parallel", "4"}, true);
}
void BuildColumn3::run(const QString& program, const QStringList& args, bool isBuild)
{
    ...
    const QString toolchain =
        findExisting({env.value("PICO_TOOLCHAIN_PATH"), QDir::home().filePath(".pico-sdk/toolchain/15_2_Rel1/bin")});
    if (!sdk.isEmpty())
        env.insert("PICO_SDK_PATH", sdk);
    if (!toolchain.isEmpty())
        env.insert("PICO_TOOLCHAIN_PATH", toolchain);
